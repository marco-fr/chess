...
class ZHash{
    public:
    void init_z_hash();
    U64 hash_board_U64(Bitboard *b, int color, int depth);
...
void ZHash::init_z_hash(){
    ...
    for(auto &i : depth_table) i = Magic::random_U64();
}
...
int ZHash::is_empty(U64 key, Bitboard *b, int depth){
    ...
    if(b->board[W_PIECES] != hash_table[new_key].white) return 1;
    if(b->board[B_PIECES] != hash_table[new_key].black) return 1;
    if(depth != hash_table[new_key].depth) return 1;
    return 0;
}

U64 ZHash::resized_key(U64 key){
    return key & mask;
}
...
void ZHash::output_size(){
    ...
    std::cout << "Total hash size (bytes): " << number_entries * sizeof(entry) << std::endl;
}
