...
class Move{
    ...
    U64 find_all_attacks_from_piece(U64 piece, int color, U64 (Move::*function) (U64, int));
...
void Move::generate_file_rank(){
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
            if(j == 0) FILE_A |= (1ULL << (i * 8 + j));
            if(j == 0 || j == 1) FILE_AB |= (1ULL << (i * 8 + j));
            if(j == 7) FILE_H |= (1ULL << (i * 8 + j));
            if(j == 7 || j == 6) FILE_GH |= (1ULL << (i * 8 + j));
            if(i == 1) RANK_7|= (1ULL << (i * 8 + j));
...
U64 Move::find_queen_attacks(U64 square, int color){
    U64 attacks = (find_bishop_attacks(square, color) | find_rook_attacks(square, color));
...
U64 Move::find_pawn_attacks(U64 square, int color){
    U64 attacks;
    if(color == WHITE){
        attacks = (((square >> 7) & ~FILE_A) |
                    ((square >> 9) & ~FILE_H));
    }
    else{
        attacks = (((square << 7) & ~FILE_H) |
                    ((square << 9) & ~FILE_A));
    }
...
U64 Move::find_all_attacks_from_piece(U64 piece, int color, U64 (Move::*function)(U64, int)){
    int square;
    U64 result = 0ULL;
    while(piece != 0ULL){
...
U64 Move::remove_color(U64 board, int color){
    if(color == WHITE) return (board & ~(curBoard->board[W_PIECES]));
