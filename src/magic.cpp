...
const U64 Magic::bishop_mask[64] = {
...
const U64 Magic::bishop_magics[64] = {
    (0x420C80100408202ULL),  (0x1204311202260108ULL), (0x2008208102030000ULL),
    (0x24081001000CAULL),    (0x488484041002110ULL),  (0x1A080C2C010018ULL),
    (0x20A02A2400084ULL),    (0x440404400A01000ULL),  (0x8931041080080ULL),
    (0x200484108221ULL),     (0x80460802188000ULL),   (0x4000090401080092ULL),
    (0x4000011040A00004ULL), (0x20011048040504ULL),   (0x2008008401084000ULL),
    (0x102422A101A02ULL),    (0x2040801082420404ULL), (0x8104900210440100ULL),
    (0x202101012820109ULL),  (0x248090401409004ULL),  (0x44820404A00020ULL),
    (0x40808110100100ULL),   (0x480A80100882000ULL),  (0x184820208A011010ULL),
    (0x110400206085200ULL),  (0x1050010104201ULL),    (0x4008480070008010ULL),
    (0x8440040018410120ULL), (0x41010000104000ULL),   (0x4010004080241000ULL),
    (0x1244082061040ULL),    (0x51060000288441ULL),   (0x2215410A05820ULL),
    (0x6000941020A0C220ULL), (0xF2080100020201ULL),   (0x8010020081180080ULL),
    (0x940012060060080ULL),  (0x620008284290800ULL),  (0x8468100140900ULL),
    (0x418400AA01802100ULL), (0x4000882440015002ULL), (0x420220A11081ULL),
    (0x401A26030000804ULL),  (0x2184208000084ULL),    (0xA430820A0410C201ULL),
    (0x640053805080180ULL),  (0x4A04010A44100601ULL), (0x10014901001021ULL),
    (0x422411031300100ULL),  (0x824222110280000ULL),  (0x8800020A0B340300ULL),
    (0xA8000441109088ULL),   (0x404000861010208ULL),  (0x40112002042200ULL),
    (0x2141006480B00A0ULL),  (0x2210108081004411ULL), (0x2010804070100803ULL),
    (0x7A0011010090AC31ULL), (0x18005100880400ULL),   (0x8010001081084805ULL),
    (0x400200021202020AULL), (0x4100342100A0221ULL),  (0x404408801010204ULL),
    (0x6360041408104012ULL)};

const U64 Magic::rook_magics[64] = {
...
U64 Magic::bishop_hash[64][4096];
U64 Magic::rook_hash[64][4096];

int Magic::num_of_ones(U64 x) { return __builtin_popcountll(x); }
...
U64 Magic::generate_occupancy(int index, int bits, U64 mask)
{
    int i, j;
    U64 result = 0ULL;
...
U64 Magic::hash_magic(U64 board, U64 magic, int bits)
{
    return ((board * magic) >> (64 - bits));
}

U64 Magic::rook_attacks(int sq, U64 block)
{
    U64 result = 0ULL;
...
U64 Magic::bishop_attacks(int sq, U64 block)
{
    U64 result = 0ULL;
    int row = sq / 8, col = sq % 8, r, f;
...
void Magic::generate_bishop_magics()
{
    U64 magic;
...
void Magic::generate_rook_magics()
{
    U64 magic;
...
void Magic::init_hashes()
{
    init_bishop_hash();
    init_rook_hash();
}

U64 Magic::find_bishop_moves(U64 sq, U64 all_pieces_mask)
{
...
U64 Magic::find_rook_moves(U64 sq, U64 all_pieces_mask)
{
    int square = pop_first_bit(&sq);
...
U64 Magic::generate_square_magic(int square, int bishop)
{
    ...
    {
        occupancies[index] =
            generate_occupancy(index, relevant_bits, attack_mask);
        attacks[index] = (bishop) ? bishop_attacks(square, occupancies[index])
                                  : rook_attacks(square, occupancies[index]);
    }

    for (i = 0; i < 10000000; i++)
    {
        magic = random_U64_fewbits();
        if (num_of_ones((attack_mask * magic) & 0xFF00000000000000ULL) < 6)
            continue;
